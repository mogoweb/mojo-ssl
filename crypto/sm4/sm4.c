...
OPENSSL_INLINE uint32_t rotl(uint32_t a, uint8_t n)
{
  return (a << n) | (a >> (32 - n));
}

OPENSSL_INLINE uint32_t load_u32_be(const uint8_t *b, uint32_t n)
{
...
OPENSSL_INLINE void store_u32_be(uint32_t v, uint8_t *b)
{
  b[0] = (uint8_t)(v >> 24);
  b[1] = (uint8_t)(v >> 16);
  b[2] = (uint8_t)(v >> 8);
...
OPENSSL_INLINE uint32_t SM4_T_slow(uint32_t X)
{
  uint32_t t = 0;
...
OPENSSL_INLINE uint32_t SM4_T(uint32_t X)
{
